...
public class DBSet<TEntity>
{
        ...
        try (PreparedStatement statement = conn.prepareStatement(query.toString(), Statement.RETURN_GENERATED_KEYS))
        {
            ...
            if (count == 1 && result.next())
            {
                ...
                ArrayList<FieldEntry> entries = fields.stream()
                                                      // Keeps the autogenerated primary key fields
    ...
    public List<TEntity> readEntity(NameValuePair... queryConditions)
    {
        ...
        try (PreparedStatement statement = conn.prepareStatement(query.toString()))
        {
            ...
            {
                // Use reflection to assign the read values to the entity
    ...
    public void updateEntity(TEntity entity)
    {
        try
        {
            List<FieldEntry> keyFields = fields.stream()
                                               .filter(FieldEntry::isPrimaryKey)
    ...
    public void updateEntity(TEntity entity, NameValuePair... queryConditions)
    {
        ...
        try
        {
            ...
            List<FieldEntry> updatableFields = fields.stream()
                                                     // Filters out all the primary keys and non-updatable fields set
                                                     // using the @NotUpdatable annotation
